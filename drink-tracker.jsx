...
export default function DrinkTracker() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const savedName = await window.storage.get('userName');
      if (savedName) {
        setUserName(savedName.value);
      }

      const sharedDrinks = await window.storage.get('allDrinks', true);
      if (sharedDrinks) {
        const parsed = JSON.parse(sharedDrinks.value);
        setAllDrinks(parsed);
        if (savedName) {
          setDrinks(parsed.filter(d => d.userName === savedName.value));
        }
      }
    } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await window.storage.set('allDrinks', JSON.stringify(updatedAllDrinks), true);
    } catch (e) {
      console.log('Could not save to storage');
    }
